...
(function () {
    ...
    function openModal() {
        ...
        document.body.classList.add('no-scroll'); /* PageSpeed fix: evita forced reflow */
        setTimeout(function () {
    ...
    function acceptAll() {
        ...
        save(state);
        dispatch(state);
        updateFooterToggleIcon(); // Atualiza toggle no rodapé
        hideBanner();
        closeModal();
    ...
    function updateFooterToggleIcon() {
        ...
        if (prefs && prefs.decided) {
            // Se aceitou algum cookie opcional, mostra como ativo
            if (prefs.functional || prefs.analytics || prefs.performance || prefs.advertising) {
                toggle.classList.remove('inactive');
                toggle.classList.add('active');
            } else {
                // Se rejeitou todos os opcionais, mostra como inativo
                toggle.classList.remove('active');
                toggle.classList.add('inactive');
            }
        } else {
            // Estado padrão (não decidiu) - mostrar como ativo
            toggle.classList.add('active');
        }
    }
    ...
    function bindEvents() {
        ...
        on('ck-prefs-link-footer', 'click', function (e) {
            e.preventDefault();
            openModal();
        });
    ...
    function init() {
        ...
        if (saved && saved.decided && !isExpired(saved.timestamp)) {
            ...
            dispatch(state);
            updateFooterToggleIcon(); // Inicializa o toggle
            if (CONFIG.showFloatingBtn) showFloatingBtn();
            return;
        }
        
        updateFooterToggleIcon(); // Inicializa o toggle como ativo (default)
